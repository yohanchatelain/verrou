...
public:
  myLibMathFunction1(std::string name, uint64_t enumName)
      : name_(name), hash_(enumName + nbOpHash) {
...
public:
  myLibMathFunction2(std::string name, uint64_t enumName)
      : name_(name), hash_(enumName + nbOpHash) {
    load_real_sym((void **)&(real_name_float), name + std::string("f"));
    load_real_sym((void **)&(real_name_double), name);
    load_real_sym((void **)&(real_name_long_double), name + std::string("l"));
...
// shell LIST1="acos acosh asin asinh atan atanh cbrt erf exp exp2 expm1 log
// log10 log1p log2 tgamma lgamma sin sinh cos cosh sqrt tan tanh j0 j1 y0 y1"
...
myLibMathFunction1 function1NameTab[enum_libm_function1_name_size] = {
    // shell command to generate  for i in $LIST1 ; do  echo
    // "myLibMathFunction1(\"$i\", enum$i),"; done;
...
template <class> struct realTypeIndex;
template <> struct realTypeIndex<float> { static const int index = 0; };
template <> struct realTypeIndex<double> { static const int index = 1; };
template <> struct realTypeIndex<long double> { static const int index = 2; };

template <class REALTYPE, int ENUM_LIBM, int INST> inline void incCounter1() {
  libMathCounter1[ENUM_LIBM][realTypeIndex<REALTYPE>::index][INST]++;
}

template <class REALTYPE, int ENUM_LIBM, int INST> inline void incCounter2() {
...
const char *verrou_rounding_mode_name_redefined(enum vr_RoundingMode mode) {
  ...
  case VR_NATIVE:
    return "NATIVE";
  case VR_FTZ:
    std::cerr << "Rounding VR_FTZ not yet implemented in interlibmath"
              << std::endl;
    exit(1);
    return "FTZ";
  }
...
void printCounter() {
  std::cerr << "==" << my_pid << "== "
            << "ROUNDINGMODE: "
            << verrou_rounding_mode_name_redefined(ROUNDINGMODE) << std::endl;
  std::cerr << "==" << my_pid << "== "
            << "Interlibm counter " << std::endl;
  std::cerr << "==" << my_pid << "== "
            << "\t\t Total \tInstrumented" << std::endl;
    ...
    for (int i = 0; i < paramSize; i++) {
      ...
      if (total != 0) {
        ...
        std::cerr << " `-"
                     " flt ";
        std::cerr << "\t"
                  << getCounter(nbParam, i, 0, 0) + getCounter(nbParam, i, 0, 1)
                  << "\t" << getCounter(nbParam, i, 0, 0) << std::endl;

        std::cerr << "==" << my_pid << "== ";
        std::cerr << " `-"
                     " dbl ";
        std::cerr << "\t"
                  << getCounter(nbParam, i, 1, 0) + getCounter(nbParam, i, 1, 1)
                  << "\t" << getCounter(nbParam, i, 1, 0) << std::endl;

        std::cerr << "==" << my_pid << "== ";
        std::cerr << " `-"
                     " lgd ";
        std::cerr << "\t"
                  << getCounter(nbParam, i, 2, 0) + getCounter(nbParam, i, 2, 1)
                  << "\t" << getCounter(nbParam, i, 2, 0) << std::endl;
      }
    }
  }
}

template <class LIBMQ, typename REALTYPE> class libMathFunction1 {
public:
  ...
  static inline PackArgs comdetPack(const PackArgs &p) { return p; }
...
template <class LIBMQ, typename REALTYPE> class libMathFunction2 {
public:
  typedef REALTYPE RealType;
...
#define DEFINE_INTERP_LIBM1_C_IMPL(FCT)                                        \
  ...
  double FCT(double a) {                                                       \
    if (ROUNDINGMODE == VR_NATIVE) {                                           \
      ...
      typedef OpWithSelectedRoundingMode<libMathFunction1<libmq##FCT, double>> \
          Op;                                                                  \
      double res;                                                              \
  ...
  float FCT##f(float a) {                                                      \
    if (ROUNDINGMODE == VR_NATIVE) {                                           \
      ...
      VERROU_STOP_INSTRUMENTATION;                                             \
      typedef OpWithSelectedRoundingMode<libMathFunction1<libmq##FCT, float>>  \
          Op;                                                                  \
      float res;                                                               \
      Op::apply(Op::PackArgs(a), &res, NULL);                                  \
      VERROU_START_INSTRUMENTATION;                                            \
...
#define DEFINE_INTERP_LIBM2_C_IMPL(FCT)                                        \
  struct libmq##FCT {                                                          \
    static __float128 apply(__float128 a, __float128 b) {                      \
      return FCT##q(a, b);                                                     \
    }                                                                          \
    static __uint64_t getHash() { return enum##FCT; }                          \
  };                                                                           \
  ...
  double FCT(double a, double b) {                                             \
    if (ROUNDINGMODE == VR_NATIVE) {                                           \
      ...
      typedef OpWithSelectedRoundingMode<libMathFunction2<libmq##FCT, double>> \
          Op;                                                                  \
      VERROU_STOP_INSTRUMENTATION;                                             \
      double res;                                                              \
  ...
  float FCT##f(float a, float b) {                                             \
    if (ROUNDINGMODE == VR_NATIVE) {                                           \
      ...
      typedef OpWithSelectedRoundingMode<libMathFunction2<libmq##FCT, float>>  \
          Op;                                                                  \
      float res;                                                               \
      VERROU_STOP_INSTRUMENTATION;                                             \
      Op::apply(Op::PackArgs(a, b), &res, NULL);                               \
...
void __attribute__((destructor)) finalyze_interlibmath() { printCounter(); };
