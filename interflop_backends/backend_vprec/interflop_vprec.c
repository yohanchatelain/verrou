...
#define MALLOC(X) VG_(malloc)("", X)
#define FREE(X) VG_(free)(X)
#define STRCASECMP(X, Y) VG_(strcasecmp)(X, Y)
#define STRCMP(X, Y) VG_(strcmp)(X, Y)
#define GETENV(X) VG_(getenv)(X)
#define abs(x) (~0x80000000 & x)
static int strtoll10(const char *nptr, char **endptr, int *error) {
...
void logger_error(const char *fmt, ...) {
  ...
  va_start(ap, fmt);
  VG_(vprintf)(fmt, ap);
  if (vprec_panicHandler != NULL) {
    vprec_panicHandler("VPREC BACKEND ERROR");
  }
}
...
const char *vprec_backend_name = "vprec";
const char *vprec_backend_version = "1.x-dev";
...
static void _set_vprec_mode(vprec_mode mode) {
  if (mode >= _vprecmode_end_) {
...
static void _set_vprec_precision_binary32(int precision) {
...
static void _set_vprec_range_binary32(int range) {
...
static void _set_vprec_precision_binary64(int precision) {
...
static void _set_vprec_range_binary64(int range) {
...
#ifndef USE_VERROU_VPREC
static void _set_vprec_input_file(const char *input_file) {
  vprec_input_file = input_file;
}

static void _set_vprec_output_file(const char *output_file) {
  vprec_output_file = output_file;
}

static void _set_vprec_log_file(const char *log_file) {
...
static void _set_vprec_inst_mode(vprec_inst_mode mode) {
...
static vprec_preset _get_vprec_preset(const char *preset) {
  if (STRCMP(VPREC_PRESET_STR[preset_binary16], preset) == 0) {
    return preset_binary16;
  } else if (STRCMP(VPREC_PRESET_STR[preset_binary32], preset) == 0) {
    return preset_binary32;
  } else if (STRCMP(VPREC_PRESET_STR[preset_bfloat16], preset) == 0) {
    return preset_bfloat16;
  } else if (STRCMP(VPREC_PRESET_STR[preset_tensorfloat], preset) == 0) {
    return preset_tensorfloat;
  } else if (STRCMP(VPREC_PRESET_STR[preset_fp24], preset) == 0) {
    return preset_fp24;
  } else if (STRCMP(VPREC_PRESET_STR[preset_PXR24], preset) == 0) {
    return preset_PXR24;
  } else {
...
static vprec_preset_precision _get_vprec_preset_precision(vprec_preset preset) {
  switch (preset) {
  case preset_binary16:
    return preset_precision_binary16;
  case preset_binary32:
    return preset_precision_binary32;
  case preset_bfloat16:
    return preset_precision_bfloat16;
  case preset_tensorfloat:
    return preset_precision_tensorfloat;
  case preset_fp24:
    return preset_precision_fp24;
  case preset_PXR24:
    return preset_precision_PXR24;
  default:
    logger_error("invalid preset provided, must be one of: "
                 "{binary16, binary32, binary64, bfloat16, tensorfloat, "
                 "fp24, PXR24}");
    return _preset_precision_end_;
  }
}
...
static vprec_preset_range _get_vprec_preset_range(vprec_preset preset) {
  switch (preset) {
  case preset_binary16:
    return preset_range_binary16;
  case preset_binary32:
    return preset_range_binary32;
  case preset_bfloat16:
    return preset_range_bfloat16;
  case preset_tensorfloat:
    return preset_range_tensorfloat;
  case preset_fp24:
    return preset_range_fp24;
  case preset_PXR24:
    return preset_range_PXR24;
  default:
    logger_error("invalid preset provided, must be one of: "
                 "{binary16, binary32, binary64, bfloat16, tensorfloat, "
                 "fp24, PXR24}");
    return _preset_range_end_;
  }
}

static vprec_err_mode _get_vprec_error_mode(const char *arg) {
  if (STRCASECMP(VPREC_ERR_MODE_STR[vprec_err_mode_rel], arg) == 0) {
    return vprec_err_mode_rel;
  } else if (STRCASECMP(VPREC_ERR_MODE_STR[vprec_err_mode_abs], arg) == 0) {
    return vprec_err_mode_abs;
  } else if (STRCASECMP(VPREC_ERR_MODE_STR[vprec_err_mode_all], arg) == 0) {
    return vprec_err_mode_all;
  } else {
    return _vprec_err_end_;
  }
}

static void _set_vprec_error_mode(vprec_err_mode error_mode, void *context) {
...
static void _set_vprec_absolute_error_exponent(long abs_err_exponent,
                                               void *context) {
  t_context *ctx = (t_context *)context;
  ctx->absErr_exp = abs_err_exponent;
}

static void _set_vprec_daz(void *context) {
  t_context *ctx = (t_context *)context;
  ctx->daz = true;
}

static void _set_vprec_ftz(void *context) {
  t_context *ctx = (t_context *)context;
  ctx->ftz = true;
}
...
static inline float _vprec_binary32_ternary_op(float a, float b, float c,
  ...
  if (VPRECLIB_MODE == vprecmode_ib || VPRECLIB_MODE == vprecmode_full) {
    a = _vprec_round_binary32(a, 0, context, VPRECLIB_BINARY32_PRECISION,
                              VPRECLIB_BINARY32_RANGE);
    b = _vprec_round_binary32(b, 0, context, VPRECLIB_BINARY32_PRECISION,
                              VPRECLIB_BINARY32_RANGE);
    c = _vprec_round_binary32(c, 0, context, VPRECLIB_BINARY32_PRECISION,
                              VPRECLIB_BINARY32_RANGE);
  }

  float res = fmaApprox(a, b, c);

  if (VPRECLIB_MODE == vprecmode_ob || VPRECLIB_MODE == vprecmode_full) {
    res = _vprec_round_binary32(res, 0, context, VPRECLIB_BINARY32_PRECISION,
                                VPRECLIB_BINARY32_RANGE);
  }
...
static inline double _vprec_binary64_ternary_op(double a, double b, double c,
  ...
  if (VPRECLIB_MODE == vprecmode_ib || VPRECLIB_MODE == vprecmode_full) {
    a = _vprec_round_binary64(a, 0, context, VPRECLIB_BINARY64_PRECISION,
                              VPRECLIB_BINARY64_RANGE);
    b = _vprec_round_binary64(b, 0, context, VPRECLIB_BINARY64_PRECISION,
                              VPRECLIB_BINARY64_RANGE);
    c = _vprec_round_binary64(c, 0, context, VPRECLIB_BINARY64_PRECISION,
                              VPRECLIB_BINARY64_RANGE);
  }

  double res = fmaApprox(a, b, c);

  if (VPRECLIB_MODE == vprecmode_ob || VPRECLIB_MODE == vprecmode_full) {
    res = _vprec_round_binary64(res, 0, context, VPRECLIB_BINARY64_PRECISION,
                                VPRECLIB_BINARY64_RANGE);
  }
...
const char *interflop_vprec_get_backend_name(void) {
  return vprec_backend_name;
}

const char *interflop_vprec_get_backend_version(void) {
  return vprec_backend_version;
}
...
void interflop_vprec_cast_double_to_float(double a, float *b, void *context) {
  *b = _vprec_round_binary64(a, 0, context, VPRECLIB_BINARY32_PRECISION,
                             VPRECLIB_BINARY32_RANGE);
}
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  case KEY_PREC_B32:
    /* precision */
    val = strtoll10(arg, &endptr, &error);
  ...
  case KEY_INPUT_FILE:
    /* input file */
#ifdef USE_VERROU_VPREC
    logger_error("Unsupported option when using Verrou frontend");
#else
  ...
  case KEY_MODE:
    ...
    break;
  case KEY_ERR_MODE:
    vprec_err_mode mode = _get_vprec_error_mode(arg);
    _set_vprec_error_mode(mode, ctx);
    break;
  case KEY_ERR_EXP:
    /* exponent of the maximum absolute error */
    long absErr_exp = strtoll10(arg, &endptr, &error);
    _set_vprec_absolute_error_exponent(absErr_exp, ctx);
    if (error != 0) {
  ...
  case KEY_FTZ:
    ...
    break;
  case KEY_PRESET:
    vprec_preset preset = _get_vprec_preset(arg);
    precision = _get_vprec_preset_precision(preset);
    range = _get_vprec_preset_range(preset);
...
void interflop_vprec_configure(vprec_conf_t conf, void *context) {
  ...
  _set_vprec_precision_binary32(precision_binary32);
  _set_vprec_precision_binary64(precision_binary64);
  _set_vprec_range_binary32(range_binary32);
  _set_vprec_range_binary64(range_binary64);
  _set_vprec_mode(conf.mode);
  _set_vprec_error_mode(conf.err_mode, context);
  if (conf.max_abs_err_exponent != (unsigned int)(-1)) {
    _set_vprec_absolute_error_exponent(conf.max_abs_err_exponent, context);
  }
  if (conf.daz) {
    _set_vprec_daz(context);
  }
  if (conf.ftz) {
    _set_vprec_ftz(context);
  }
}
...
static void print_information_header(void *context) {
...
#ifndef USE_VERROU_VPREC
  const char *key_inst_value = VPREC_INST_MODE_STR[VPREC_INST_MODE];
#else
  const char *key_inst_value = "<undefined>";
#endif
  ...
  logger_info(
      ...
      (ctx->relErr && !ctx->absErr)   ? VPREC_ERR_MODE_STR[vprec_err_mode_rel]
      : (!ctx->relErr && ctx->absErr) ? VPREC_ERR_MODE_STR[vprec_err_mode_abs]
      : (ctx->relErr && ctx->absErr)  ? VPREC_ERR_MODE_STR[vprec_err_mode_all]
      ...
      key_instrument_str, key_inst_value);
}

void interflop_vprec_finalize(__attribute__((unused)) void *context) {
#ifndef USE_VERROU_VPREC
  /* save the hashmap */
...
struct interflop_backend_interface_t interflop_vprec_init(int argc, char **argv,
  ...
  *context = ctx;
  ...
  struct interflop_backend_interface_t interflop_backend_vprec = {
      ...
      interflop_vprec_add_float,
      interflop_vprec_sub_float,
      interflop_vprec_mul_float,
      interflop_vprec_div_float,
      NULL,
      interflop_vprec_add_double,
      interflop_vprec_sub_double,
      interflop_vprec_mul_double,
      interflop_vprec_div_double,
      NULL,
      interflop_vprec_cast_double_to_float,
      interflop_vprec_madd_double,
      interflop_vprec_madd_float,
      interflop_vprec_enter_function,
      interflop_vprec_exit_function,
      interflop_vprec_user_call,
      interflop_vprec_finalize};
