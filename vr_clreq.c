...
void vr_set_instrument_state(const HChar *reason, Vr_Instr state,
  ...
  if (vr.instrument == state) {
    if (vr.verbose) {
      VG_(message)
      (Vg_DebugMsg, "%s: instrumentation already %s\n", reason,
       (state == VR_INSTR_ON) ? "ON" : "OFF");
    }
  ...
  if (vr.verbose) {
    VG_(message)
    (Vg_DebugMsg, "%s: instrumentation switched %s\n", reason,
     (state == VR_INSTR_ON) ? "ON" : "OFF");
  }
}

Bool vr_instrumentation_is_on(void) { return vr.instrument == VR_INSTR_ON; }

// * Enter/leave deterministic section

static void vr_deterministic_section_name(unsigned int level, HChar *name,
                                          unsigned int len) {
  Addr ips[8];
  const HChar *fnname;
  ...
  VG_(get_StackTrace)(VG_(get_running_tid)(), ips, 8, NULL, NULL, 0);
  addr = ips[level];
  ...
  //  filename[0] = 0;
  VG_(get_filename_linenum)(de, addr, &filename, NULL, &linenum);
  VG_(snprintf)(name, len, "%s (%s:%u)", fnname, filename, linenum);
}
...
#endif
  VG_(message)
  (Vg_DebugMsg, "Entering deterministic section %llu: %s\n", hash, name);
}
...
static Bool vr_handle_monitor_command(HChar *req) {
  ...
  switch (VG_(keyword_id)("help instrumentation count", wcmd,
...
#else
    VG_(umsg)
    ("Warning : verrou need to be compiled with "
     "--enable-verrou-profiling-exact to use the client request "
  ...
  case VR_USERREQ__PRANDOM_UPDATE_VALUE: {
    ...
    if (vr.verbose) {
      if ((vr.roundingMode == VR_PRANDOM) ||
          ...
          (vr.roundingMode == VR_PRANDOM_COMDET)) {
        VG_(umsg)
        ("\tPRANDOM_UPDATE_VALUE: pvalue=%f\n", verrou_prandom_pvalue());
      }
    }
    break;
  case VR_USERREQ__INSTRUMENTATION_IS_ON:
    *ret = vr_instrumentation_is_on();
    break;
  }
  return True;
}
