...
static Vr_Include_Trace *vr_addIncludeTrace(Vr_Include_Trace *list,
                                            const HChar *fnname,
                                            const HChar *objname) {
  Vr_Include_Trace *cell =
      VG_(malloc)("vr.addIncludeTrace.1", sizeof(Vr_Include_Trace));
...
static Vr_Include_Trace *vr_findIncludeTrace(Vr_Include_Trace *list,
                                             const HChar *fnname,
                                             const HChar *objname) {
  Vr_Include_Trace *include;
  for (include = list; include != NULL; include = include->next) {
    if (include->fnname[0] != '*' && VG_(strcmp)(include->fnname, fnname) != 0)
      continue;
...
Bool vr_includeTraceIRSB(const HChar **fnname, const HChar **objname) {
  ...
  Vr_Include_Trace *include =
      vr_findIncludeTrace(vr.includeTrace, *fnname, *objname);
  if (include != NULL) {
    return True;
  }
  return False;
