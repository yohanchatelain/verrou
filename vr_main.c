...
#define bcNameWithCC(OP)                                                       \
  "vr_verroucheck_float_max" #OP, vr_verroucheck_float_max##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
#define bcName(OP) "vr_checkdenorm" #OP, vr_checkdenorm##OP
#define bcNameWithCC(OP)                                                       \
  "vr_checkdenormcheckcancellation" #OP, vr_checkdenormcheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
static void vr_post_clo_init(void) {
  ...
  interflop_checkcancellation_configure(checkcancellation_conf,
  ...
  backend_check_float_max =
      interflop_check_float_max_init(&backend_check_float_max_context);
  ifmax_set_max_handler(&vr_handle_FLT_MAX);
  ifmax_set_debug_print_op(
      &print_op); // Use only verrou backend is configured to use it

  VG_(umsg)
  ("Backend %s : %s\n", interflop_check_float_max_get_backend_name(),
   interflop_check_float_max_get_backend_version());
  ...
  backend_checkdenorm =
      interflop_checkdenorm_init(&backend_checkdenorm_context);
  interflop_checkdenorm_configure(checkdenorm_conf,
                                  backend_checkdenorm_context);
  checkdenorm_set_denorm_handler(&vr_handle_CD);
  checkdenorm_set_panic_handler(&VG_(tool_panic));
  VG_(umsg)
  ("Backend %s : %s\n", interflop_checkdenorm_get_backend_name(),
   interflop_checkdenorm_get_backend_version());
  ...
  if (vr.backend == vr_verrou) {
    VG_(umsg)
    ("Backend verrou simulating %s rounding mode\n",
     verrou_rounding_mode_name(vr.roundingMode));
  }
  if (vr.backend == vr_mcaquad) {
#ifdef USE_VERROU_QUAD
    VG_(umsg)
    ("Backend mcaquad simulating mode %s with precision %u for double and %u "
     "for float\n",
     mcaquad_mode_name(vr.mca_mode), vr.mca_precision_double,
  ...
  if (vr.backend == vr_vprec) {
    VG_(umsg)
    ("Backend vprec simulating mode %s with format (precision,range) (%u,%u) "
     "for double and (%u,%u) for float\n",
     get_vprec_mode_name(vr.vprec_mode), vr.vprec_precision_binary64,
     vr.vprec_range_binary64, vr.vprec_precision_binary32,
