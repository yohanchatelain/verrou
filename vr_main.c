...
#include "float.h"
#include "interflop_backends/interflop_bitmask/interflop_valgrind_bitmask.h"
#include "interflop_backends/interflop_cancellation/interflop_valgrind_cancellation.h"
#include "interflop_backends/interflop_checkcancellation/interflop_valgrind_checkcancellation.h"
#include "interflop_backends/interflop_checkdenormal/interflop_valgrind_checkdenormal.h"
#include "interflop_backends/interflop_checkfloatmax/interflop_valgrind_checkfloatmax.h"
#include "interflop_backends/interflop_mcaint/interflop_valgrind_mcaint.h"
#include "interflop_backends/interflop_mcaquad/interflop_valgrind_mcaquad.h"
...
#ifdef LINK_INTERFLOP_BITMASK
#include "interflop/interflop_bitmask.h"
#endif
#ifdef LINK_INTERFLOP_CANCELLATION
#include "interflop/interflop_cancellation.h"
#endif
#ifdef LINK_INTERFLOP_CHECKCANCELLATION
...
static VG_REGPARM(2) void vr_incUnstrumentedOpCount(ULong *counter,
                                                    SizeT increment) {
  counter[VR_INSTR_OFF] += increment;
}

static VG_REGPARM(0) void vr_updatep_prandom(void) { verrou_updatep_prandom(); }
...
static void vr_countOp(IRSB *sb, Vr_Op op, Vr_Prec prec, Vr_Vec vec,
  ...
  if (instr) {
    ...
    di = unsafeIRDirty_0_N(2, "vr_incOpCount",
                           VG_(fnptr_to_fnentry)(&vr_incOpCount), argv);

  } else {
    argv = mkIRExprVec_2(mkIRExpr_HWord((HWord)&vr_opCount[op][prec][vec]),
                         mkIRExpr_HWord(increment));

    di = unsafeIRDirty_0_N(2, "vr_incUnstrumentedOpCount",
...
UInt vr_count_fp_instrumented(void) {
  // Warning : the return type is small (to be consistent with return client
  // request type) should be used only  as heuristic  to detect dynamicaly fp
  // operations
  if (!vr.count) {
    VG_(tool_panic)
    ("--count-op=no not allowed with vr_count_fp_instrumented function \n");
  }
  Vr_Op op;
  Vr_Prec prec;
  Vr_Vec vec;
...
UInt vr_count_fp_not_instrumented(void) {
  ...
  if (!vr.count) {
    VG_(tool_panic)
    ("--count-op=no not allowed with vr_count_fp_not_instrumented function \n");
  }
  Vr_Op op;
  Vr_Prec prec;
  Vr_Vec vec;
...
void vr_ppOpCount(void) {
  ...
  for (op = 0; op < VR_OP; ++op) {
    ...
    if (countOp[VR_INSTR_ON] + countOp[VR_INSTR_OFF] > 0) {
      VG_(umsg)
      (" %-6s       %15llu          %15llu          (%3u%%)\n", vr_ppOp(op),
       countOp[VR_INSTR_ON] + countOp[VR_INSTR_OFF], countOp[VR_INSTR_ON],
      ...
      for (prec = 0; prec < VR_PREC; ++prec) {
        ...
        if (countPrec[VR_INSTR_ON] + countPrec[VR_INSTR_OFF] > 0) {
          VG_(umsg)
          ("  `- %-8s     %15llu          %15llu      (%3u%%)\n",
           vr_ppPrec(prec), countPrec[VR_INSTR_ON] + countPrec[VR_INSTR_OFF],
            ...
            if (count[VR_INSTR_ON] + count[VR_INSTR_OFF] > 0) {
              ...
              ("      `- %-6s       %15llu          %15llu  (%3u%%)\n",
               ...
               vr_opCount[op][prec][vec][VR_INSTR_ON] +
                   vr_opCount[op][prec][vec][VR_INSTR_OFF],
               vr_opCount[op][prec][vec][VR_INSTR_ON],
               vr_frac(vr_opCount[op][prec][vec][VR_INSTR_ON],
                       vr_opCount[op][prec][vec][VR_INSTR_OFF]));
            }
          }
        }
...
static Bool vr_isInstrumented(Vr_Op op, Vr_Prec prec, Vr_Vec vec) {
  return vr.instr_op[op] && vr.instr_vec[vec] && vr.instr_prec[prec] &&
         vr.instrument;
}
...
static Bool vr_replaceBinFpOpScal(IRSB *sb, IRStmt *stmt, IRExpr *expr,
                                  ...
                                  Vr_Op op, Vr_Prec prec, Vr_Vec vec,
                                  Bool countOnly) {
  ...
  addStmtToIRSB(sb, IRStmt_Dirty(unsafeIRDirty_1_N(
                        res, 2, functionName, VG_(fnptr_to_fnentry)(function),
  ...
  if (prec == VR_PREC_DBL) {
    addStmtToIRSB(
        sb, IRStmt_WrTmp(stmt->Ist.WrTmp.tmp,
...
static Bool vr_replaceBinFpOpLLO_slow_safe(IRSB *sb, IRStmt *stmt, IRExpr *expr,
                                           const HChar *functionName,
                                           void *function, Vr_Op op,
                                           Vr_Prec prec, Vr_Vec vec,
                                           Bool countOnly) {
  ...
  if (prec == VR_PREC_FLT) {
    addStmtToIRSB(sb, IRStmt_WrTmp(stmt->Ist.WrTmp.tmp,
                                   IRExpr_Binop(Iop_SetV128lo32, arg1,
                                                IRExpr_RdTmp(res))));
  }
  if (prec == VR_PREC_DBL) {
    addStmtToIRSB(sb, IRStmt_WrTmp(stmt->Ist.WrTmp.tmp,
                                   IRExpr_Binop(Iop_SetV128lo64, arg1,
                                                IRExpr_RdTmp(res))));
  }
  return True;
}

static Bool vr_replaceBinFpOpLLO_fast_unsafe(
    IRSB *sb, IRStmt *stmt, IRExpr *expr, const HChar *functionName,
    void *function, Vr_Op op, Vr_Prec prec, Vr_Vec vec, Bool countOnly) {
  // instrumentation to count operation
  if (!(vr_isInstrumented(op, prec, vec))) {
    vr_countOp(sb, op, prec, vec, False);
    addStmtToIRSB(sb, stmt);
...
static Bool vr_replaceBinFpOpLLO(IRSB *sb, IRStmt *stmt, IRExpr *expr,
  ...
  if (vr.unsafe_llo_optim) {
    return vr_replaceBinFpOpLLO_fast_unsafe(sb, stmt, expr, functionName,
                                            function, op, prec, vec, countOnly);
  } else {
    return vr_replaceBinFpOpLLO_slow_safe(sb, stmt, expr, functionName,
                                          function, op, prec, vec, countOnly);
  }
}
...
static Bool vr_replaceBinFullSSE(IRSB *sb, IRStmt *stmt, IRExpr *expr,
  ...
  if (!((vec == VR_VEC_FULL2 && prec == VR_PREC_DBL) ||
  ...
  // call
  addStmtToIRSB(
      sb, IRStmt_Dirty(unsafeIRDirty_1_N(
              res, 3, "", VG_(fnptr_to_fnentry)(function),
              mkIRExprVec_5(IRExpr_VECRET(), arg1Hi, arg1Lo, arg2Hi, arg2Lo))));
  // conversion after call
...
static Bool vr_replaceBinFullAVX(IRSB *sb, IRStmt *stmt, IRExpr *expr,
  ...
  if (!((vec == VR_VEC_FULL4 && prec == VR_PREC_DBL) ||
  ...
  /* 1 call avx
    addStmtToIRSB (sb,
                 IRStmt_Dirty(unsafeIRDirty_1_N (res, 1,
                                                 functionName,
    VG_(fnptr_to_fnentry)(function), mkIRExprVec_9 (IRExpr_VECRET(),
                                                                arg1Tab[0],arg1Tab[1],
    arg1Tab[2],arg1Tab[3], arg2Tab[0],arg2Tab[1], arg2Tab[2],arg2Tab[3]
                                                                )
                                                                )));*/

  if (prec == VR_PREC_DBL) {
    addStmtToIRSB(sb, IRStmt_Dirty(unsafeIRDirty_0_N(
                          1, "vr_AvxDoubleCopyFirstArg",
                          VG_(fnptr_to_fnentry)(&vr_AvxDoubleCopyFirstArg),
                          mkIRExprVec_4(arg1Tab[0], arg1Tab[1], arg1Tab[2],
                                        arg1Tab[3]))));
  } else if (prec == VR_PREC_FLT) {
    addStmtToIRSB(sb, IRStmt_Dirty(unsafeIRDirty_0_N(
                          1, "vr_AvxFloatCopyFirstArg",
                          VG_(fnptr_to_fnentry)(&vr_AvxFloatCopyFirstArg),
                          mkIRExprVec_4(arg1Tab[0], arg1Tab[1], arg1Tab[2],
                                        arg1Tab[3]))));
  }

  addStmtToIRSB(sb, IRStmt_Dirty(unsafeIRDirty_1_N(
                        res, 1, functionName, VG_(fnptr_to_fnentry)(function),
                        mkIRExprVec_5(IRExpr_VECRET(), arg2Tab[0], arg2Tab[1],
                                      arg2Tab[2], arg2Tab[3]))));
...
static Bool vr_replaceFMA(IRSB *sb, IRStmt *stmt, IRExpr *expr,
                          const HChar *functionName, void *function, Vr_Op op,
                          Vr_Prec prec, Bool countOnly) {
  ...
  IRDirty *id =
      unsafeIRDirty_1_N(res, 3, functionName, VG_(fnptr_to_fnentry)(function),
...
static Bool vr_replaceCast(IRSB *sb, IRStmt *stmt, IRExpr *expr,
  ...
  IRDirty *id =
      unsafeIRDirty_1_N(res, 1, functionName, VG_(fnptr_to_fnentry)(function),
                        mkIRExprVec_1(arg2));
...
static Vr_instr_kind vr_instrumentOp(IRSB *sb, IRStmt *stmt, IRExpr *expr,
                                     IROp op, vr_backend_name_t bc,
                                     Bool countOnly) {
...
#undef bcNameWithCC
  ...
  if (vr.backend == vr_verrou && checkCancellation && !vr.checkFloatMax) {
#define bcName(OP) "vr_verrou" #OP, vr_verrou##OP
#define bcNameWithCC(OP)                                                       \
  "vr_verroucheckcancellation" #OP, vr_verroucheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
  if (vr.backend == vr_verrou && !checkCancellation && vr.checkFloatMax) {
#define bcName(OP) "vr_verroucheckfloatmax" #OP, vr_verroucheckfloatmax##OP
#define bcNameWithCC(OP)                                                       \
  "vr_verroucheckfloatmax" #OP, vr_verroucheckfloatmax##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
  ...
  if (vr.backend == vr_verrou) {
    VG_(tool_panic)("Verrou compiled without --with-interflop-backend-verrou");
  }
#endif

#ifdef LINK_INTERFLOP_BACKEND_CHECKDENORMAL
  if (vr.backend == vr_checkdenormal && !checkCancellation) {
#define bcName(OP) "vr_checkdenormal" #OP, vr_checkdenormal##OP
#define bcNameWithCC(OP) "vr_checkdenormal" #OP, vr_checkdenormal##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
#define bcName(OP) "vr_checkdenormal" #OP, vr_checkdenormal##OP
#define bcNameWithCC(OP)                                                       \
  "vr_checkdenormalcheckcancellation" #OP, vr_checkdenormalcheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
#else
  if (vr.backend == vr_checkdenormal) {
    VG_(tool_panic)
    ("Verrou compiled without --with-interflop-backend-checkdenormal");
  }
#endif
...
#define bcName(OP) "vr_mcaquad" #OP, vr_mcaquad##OP
#define bcNameWithCC(OP)                                                       \
  "vr_mcaquadcheckcancellation" #OP, vr_mcaquadcheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
  ...
  if (vr.backend == vr_mcaquad) {
    VG_(tool_panic)("Verrou compiled without --with-interflop-backend-mcaquad");
  }
#endif // LINK_INTERFLOP_BACKEND_MCAQUAD

#ifdef LINK_INTERFLOP_BACKEND_BITMASK
  if (vr.backend == vr_bitmask && !checkCancellation) {
#define bcName(OP) "vr_bitmask" #OP, vr_bitmask##OP
#define bcNameWithCC(OP) "vr_bitmask" #OP, vr_bitmask##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
#define bcName(OP) "vr_bitmask" #OP, vr_bitmask##OP
#define bcNameWithCC(OP)                                                       \
  "vr_bitmaskcheckcancellation" #OP, vr_bitmaskcheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
#else
  if (vr.backend == vr_bitmask) {
    VG_(tool_panic)("Verrou compiled without --with-interflop-backend-bitmask");
  }
#endif // LINK_INTERFLOP_BACKEND_BITMASK

#ifdef LINK_INTERFLOP_BACKEND_CANCELLATION
  if (vr.backend == vr_cancellation && !checkCancellation) {
#define bcName(OP) "vr_cancellation" #OP, vr_cancellation##OP
#define bcNameWithCC(OP) "vr_cancellation" #OP, vr_cancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
#define bcName(OP) "vr_cancellation" #OP, vr_cancellation##OP
#define bcNameWithCC(OP)                                                       \
  "vr_cancellationcheckcancellation" #OP, vr_cancellationcheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
#else
  if (vr.backend == vr_cancellation) {
    VG_(tool_panic)
    ("Verrou compiled without --with-interflop-backend-cancellation");
  }
#endif // LINK_INTERFLOP_BACKEND_CANCELLATION

#ifdef LINK_INTERFLOP_BACKEND_MCAINT
  if (vr.backend == vr_mcaint && !checkCancellation) {
#define bcName(OP) "vr_mcaint" #OP, vr_mcaint##OP
#define bcNameWithCC(OP) "vr_mcaint" #OP, vr_mcaint##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
#define bcName(OP) "vr_mcaint" #OP, vr_mcaint##OP
#define bcNameWithCC(OP)                                                       \
  "vr_mcaintcheckcancellation" #OP, vr_mcaintcheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
#else
  if (vr.backend == vr_mcaint) {
    VG_(tool_panic)("Verrou compiled without --with-interflop-backend-mcaint");
  }
#endif // LINK_INTERFLOP_BACKEND_MCAINT

#ifdef LINK_INTERFLOP_BACKEND_VPREC
  if (vr.backend == vr_vprec && !checkCancellation) {
#define bcName(OP) "vr_vprec" #OP, vr_vprec##OP
#define bcNameWithCC(OP) "vr_vprec" #OP, vr_vprec##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
...
#define bcName(OP) "vr_vprec" #OP, vr_vprec##OP
#define bcNameWithCC(OP)                                                       \
  "vr_vpreccheckcancellation" #OP, vr_vpreccheckcancellation##OP
#include "vr_instrumentOp_impl.h"
#undef bcName
#undef bcNameWithCC
  }
#else
  VG_(tool_panic)("Verrou compiled without --with-interflop-backend-vprec");
#endif
...
static Vr_instr_kind vr_instrumentExpr(IRSB *sb, IRStmt *stmt, IRExpr *expr,
                                       Bool countOnly) {
  IROp op;
...
inline void vr_treat_line_from_imark(Bool excludeIrsb, Bool includeSource,
                                     Bool doLineContainFloat,
                                     HChar const **fnnamePtr,
                                     const HChar **filenamePtr,
                                     const UInt *linenumPtr) {
  if (!excludeIrsb && doLineContainFloat) {
    if (vr.genIncludeSource) {
      vr_includeSource_generate(&vr.gen_includeSource, *fnnamePtr, *filenamePtr,
                                *linenumPtr);
    }

    if (!includeSource && vr.sourceActivated &&
        !vr_includeSource(&vr.excludeSourceRead, *fnnamePtr, *filenamePtr,
                          *linenumPtr)) {
      VG_(umsg)("Warning new source line with fp operation discovered :\n");
      VG_(umsg)("\t%s : %s : %u\n", *fnnamePtr, *filenamePtr, *linenumPtr);
      vr.excludeSourceRead = vr_addIncludeSource(
          vr.excludeSourceRead, *fnnamePtr, *filenamePtr, *linenumPtr);
    }
  }
}

static IRSB *vr_instrument(VgCallbackClosure *closure, IRSB *sbIn,
                           const VexGuestLayout *layout,
                           const VexGuestExtents *vge,
                           const VexArchInfo *archinfo_host, IRType gWordTy,
                           IRType hWordTy) {
  /*Recuperation of the name of symbol and object of the IRSB*/
  /*No externalized in a function because the call to get_fnname forbid other
    call in the function vr_instrument : to use fnname after the end of call you
    have to use strdup
  */
  ...
  Addr ips[256];
  VG_(get_StackTrace)(VG_(get_running_tid)(), ips, 256, NULL, NULL, 0);
  Addr addr = ips[0];
  ...
  for (i = 0; i < sbIn->stmts_used; ++i) {
    ...
    case Ist_IMark: {
      ...
      Bool success = VG_(get_filename_linenum)(VG_(current_DiEpoch)(), addrMark,
                                               filenamePtr, NULL, linenumPtr);
      ...
      if (!vr.genIncludeSource) {
        includeSource = (!vr.sourceActivated) ||
                        (vr.sourceActivated &&
                         vr_includeSource(&vr.includeSource, *fnnamePtr,
                                          *filenamePtr, *linenumPtr));
      }
      ...
      if (vr.prandomUpdate == VR_PRANDOM_UPDATE_FUNC) {
        ...
        if (VG_(get_fnname_if_entry)(de, st->Ist.IMark.addr, &localFnname)) {
          IRDirty *di;
          di = unsafeIRDirty_0_N(0, "vr_updatep_prandom",
                                 VG_(fnptr_to_fnentry)(&vr_updatep_prandom),
                                 mkIRExprVec_0());
      ...
      addStmtToIRSB(
          sbOut,
          sbIn->stmts[i]); // required to be able to use breakpoint with gdb
    } break;
    case Ist_WrTmp: {
...
#ifdef LINK_INTERFLOP_BACKEND_MCAQUAD
  if (vr.backend == vr_mcaquad) {
    interflop_valgrind_mcaquad_finalize();
  }
#endif
  interflop_checkcancellation_finalize(backend_checkcancellation_context);
  interflop_checkfloatmax_finalize(backend_checkfloatmax_context);
#ifdef LINK_INTERFLOP_BACKEND_VPREC
...
static void vr_post_clo_init(void) {
  ...
  if (vr.sourceExcludeActivated) {
    if (!vr_includeSourceMutuallyExclusive(vr.includeSource,
                                           vr.excludeSourceRead)) {
  ...
  if (vr.prandomUpdate == VR_PRANDOM_UPDATE_FUNC) {
    if ((vr.roundingMode == VR_PRANDOM_DET ||
      ...
      VG_(tool_panic)
      ("prandom dynamic update and PRANDOM_[COM]DET are incompatible");
    }
  }
...
#if LINK_INTERFLOP_BACKEND_BITMASK
  interflop_valgrind_bitmask_set_env_cli();
#endif

#if LINK_INTERFLOP_BACKEND_CANCELLATION
  interflop_valgrind_cancellation_set_env_cli();
#endif

#if LINK_INTERFLOP_BACKEND_MCAINT
  interflop_valgrind_mcaint_set_env_cli();
#endif

#if LINK_INTERFLOP_BACKEND_MCAQUAD
  interflop_valgrind_mcaquad_set_env_cli();
#endif

#if LINK_INTERFLOP_BACKEND_VPREC
  ...
  if (vr.checkDenorm || vr.dumpDenorm || vr.ftz) {
    if (vr.backend == vr_mcaquad || vr.backend == vr_vprec ||
        (vr.backend == vr_verrou &&
         !(vr.roundingMode == VR_NEAREST || vr.roundingMode == VR_FTZ ||
           vr.roundingMode == VR_NATIVE))) {
      VG_(tool_panic)("backend checkDenorm incompatible with other backend");
    }
    vr.backend = vr_checkdenormal;
  }
  if (vr.checkFloatMax && vr.backend != vr_verrou) {
    VG_(tool_panic)
    ("backend check_float_max is only compatible with verrou backend");
  }
  ...
  /*configuration of bitmask backend*/
#ifdef LINK_INTERFLOP_BACKEND_BITMASK
  if (vr.backend == vr_bitmask) {
    interflop_valgrind_bitmask_init();
  }
#endif

  /*configuration of cancellation backend*/
#ifdef LINK_INTERFLOP_BACKEND_CANCELLATION
  if (vr.backend == vr_cancellation) {
    interflop_valgrind_cancellation_init();
  }
#endif

  /*configuration of mcaint backend*/
#ifdef LINK_INTERFLOP_BACKEND_MCAINT
  if (vr.backend == vr_mcaint) {
    interflop_valgrind_mcaint_init();
  }
#endif

  /*configuration of mcaquad backend*/
#ifdef LINK_INTERFLOP_BACKEND_MCAQUAD
  if (vr.backend == vr_mcaquad) {
    interflop_valgrind_mcaquad_init();
  }
#endif
/* configuration of vprec backend */
#ifdef LINK_INTERFLOP_BACKEND_VPREC
  if (vr.backend == vr_vprec) {
  ...
  /*Init outfile cancellation*/
  if (vr.checkCancellation) {
    interflop_valgrind_checkcancellation_init();
  }

  /* Init checkfloatmax backend */
  if (vr.checkFloatMax) {
    interflop_valgrind_checkfloatmax_init();
  }

  /* Init checkDenormal backend */
  if (vr.checkDenorm || vr.ftz) {
    interflop_valgrind_checkdenormal_init();
  }
  ...
  if (!someThingInstr) {
    for (opIt = 0; opIt < VR_OP_CMP;
         opIt++) { // Instruction after VR_OP_CMP (included) are not
                   // instrumented
  ...
  if (vr.sourceExcludeActivated && !(vr.sourceActivated)) {
    VG_(tool_panic)
    ("warn-unknown-source need --source(or VERROU_SOURCE) defined ");
  }
}
...
static void vr_pre_clo_init(void) {
  ...
  ("Copyright (C) 2014-2019, EDF (F. Fevotte & B. Lathuiliere). 2019-2021, EDF "
   "(B. Lathuiliere). 2020-2021, TriScale innov (F. Fevotte)\n ");
  ...
  VG_(basic_tool_funcs)(vr_post_clo_init, vr_instrument, vr_fini);

  VG_(needs_command_line_options)
  (vr_process_clo, vr_print_usage, vr_print_debug_usage);

  VG_(needs_client_requests)(vr_handle_client_request);

  VG_(needs_tool_errors)
  (vr_eq_Error, vr_before_pp_Error, vr_pp_Error,
   False, // show_ThreadIDs_for_errors
   vr_update_extra, vr_recognised_suppression, vr_read_extra_suppression_info,
   vr_error_matches_suppression, vr_get_error_name,
   vr_print_extra_suppression_info, vr_print_extra_suppression_use,
