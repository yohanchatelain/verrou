...
// typedef enum typeInstr{ instrTrace, instrCount, instrCountAndTrace}
// typeInstr_t;

static void vr_traceIRSB(IRSB *out, UInt index,
                         UInt *counterPtr) { //, typeInstr_t select) {
  ...
  /*   break; */
  /* case instrCount: */
  di = unsafeIRDirty_0_N(2, "vr_count_dyn_IRSB",
                         VG_(fnptr_to_fnentry)(&vr_count_dyn_IRSB), argv);
  /*   break; */
...
Bool vr_is_dir(
    const HChar *f) { // copy of /coregrind/m_libcfile.c which is not public
  struct vg_stat buf;
  SysRes res = VG_(stat)(f, &buf);
  return sr_isError(res) ? False : VKI_S_ISDIR(buf.mode) ? True : False;
}
...
void vr_traceBB_initialize(char *path) {
  ...
  const HChar *strExpInfo =
      VG_(expand_file_name)("vr.traceBB.strInfo", absfileInfo);
  const HChar *strExpCov =
      VG_(expand_file_name)("vr.traceBB.strCov", absfileCov);

  vr_out_bb_info =
      VG_(fopen)(strExpInfo,
  ...
  if (/*vr_out_bb_trace==NULL || */ vr_out_bb_info ==
          NULL /*|| vr_out_bb_info_backtrace==NULL */
      || vr_out_bb_cov == NULL) {
...
void vr_traceBB_trace_imark(traceBB_t *tr, const HChar *fnname,
                            const HChar *filename, UInt lineNum,
                            Bool doLineContainFloat,
                            Bool doLineContainFloatCmp);
void vr_traceBB_trace_imark(traceBB_t *tr, const HChar *fnname,
                            const HChar *filename, UInt lineNum,
                            Bool doLineContainFloat,
                            Bool doLineContainFloatCmp) {
  VG_(fprintf)
  (vr_out_bb_info, "%u : %s : %s : %u : %s : %s\n", (tr->index), fnname,
   filename, lineNum, doLineContainFloat ? "1" : "0",
